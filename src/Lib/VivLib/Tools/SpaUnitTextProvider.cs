...
public class SpaUnitTextProvider(Carp carp) : FeDataTextProvider(carp, CultureInfo.GetCultureInfo("es-ES"))
{
    /// <inheritdoc/>
    ...
    public override string Torque
    {
        get
        {
            ...
            return $"{(torque * 1.3558179483).ToString("0", Culture)} M/Kg a {rpm.ToString(Culture)} RPM";
        }
    }
