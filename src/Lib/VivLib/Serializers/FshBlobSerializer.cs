...
public class FshBlobSerializer : ISerializer<FshBlob?>
{
    ...
    public FshBlob? Deserialize(Stream stream)
    {
        ...
        if (footerOffset != 0)
        {
            ...
            {
                if (j.Value.Invoke(footer))
                {
                    Mappings.FshFooterLoader[j.Key].Invoke(blob, footer);
                }
            }
        }
        blob.Footer = footer;
    ...
    public void SerializeTo(FshBlob? entity, Stream stream)
    {
        ...
        if (entity.Footer.Length != 0)
        {
            foreach (var j in Mappings.FshBlobFooterIdentifier)
            {
                if (j.Value.Invoke(entity.Footer))
                {
                    entity.Footer = Mappings.FshFooterWriter[j.Key].Invoke(entity);
                    break;
                }
            }
