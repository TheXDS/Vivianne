...
public class GeoEditorViewModel : StatefulFileEditorViewModelBase<GeoEditorState, GeoFile>
{
    ...
    /// <summary>
    /// Gets a reference to the command used to edit a GEO part.
    /// </summary>
    public ICommand EditGeoPartCommand { get; }
    ...
    public GeoEditorViewModel()
    {
        ...
        EditGeoPartCommand = new SimpleCommand<GeoPart>(OnEditGeoPart);
    }

    private async Task OnEditGeoPart(GeoPart? geoPart)
    {
        if (geoPart is null || DialogService is null) return;
        await DialogService.Show(new GeoPartEditorViewModel(new GeoPartEditorState(geoPart, State.FshFile)));
        State.Refresh();
    }
    ...
    private async Task OnLoadExternalFsh()
    {
        if (DialogService is not null && await DialogService.GetFileOpenPath(FileFilters.FshQfsOpenFileFilter) is { Success: true, Result: { } fshFile })
        {
            try
            {
                ...
                State.FshName = Path.GetFileName(fshFile);
                FshPreviewViewModel = new FshVm
                {
                    State = new FshEditorState() { File = State.FshFile },
                    CurrentImage = State.FshFile.Entries.Values.First(),
