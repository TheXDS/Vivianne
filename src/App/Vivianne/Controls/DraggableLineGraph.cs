...
public class DraggableLineGraph : Control
{
    ...
    static DraggableLineGraph()
    {
        ...
        DataProperty = NewDp<IList<double>, DraggableLineGraph>(nameof(Data), FrameworkPropertyMetadataOptions.AffectsRender, [], OnDataChanged);
    ...
    {
        get => (IList<double>)GetValue(DataProperty);
    ...
    {
        double x = (canvasWidth * index / (Data.Count - 1)) + 5;
    ...
    {
        double minValue = MinValue.OrIfInvalid(Data.Min());
        double maxValue = MaxValue.OrIfInvalid(Data.Max());
    ...
    {
        if (canvas == null || Data == null || !Data.Any()) return;
        ...
        pointTicks.Clear();
        Point[] points = Data.WithIndex().Select(p => CalculatePosition(p.index, p.element)).ToArray();
    ...
    private void PointTick_MouseDown(object sender, MouseButtonEventArgs e)
    {
        if (e.ChangedButton == MouseButton.Left)
        {
            ...
            if (draggingIndex.HasValue && draggingIndex.Value != -1)
            {
                ...
                initialValue = Data[draggingIndex.Value];
                canvas?.CaptureMouse();
                isDragging = true;
            }
        }
    ...
    private void Canvas_MouseMove(object sender, MouseEventArgs e)
    {
        if (isDragging && draggingIndex.HasValue && draggingIndex.Value != -1)
        {
            ...
            Data[draggingIndex.Value] = newValue;
            UpdateVisuals();
        }
    }
