...
public class FshCompressTool : IVivianneTool
{
    ...
    {
        var fin = await dialogService.GetFileOpenPath(StC.Open, St.OpnMessage, FileFilters.FshFileFilter);
        if (!fin.Success) return;
        var fout = await dialogService.GetFileSavePath(StC.Save, St.SveMessage, FileFilters.QfsFileFilter);
