...
public class QfsDecompressTool : IVivianneTool
{
    ...
    {
        var fin = await dialogService.GetFileOpenPath(St.Open, St2.OpnMessage, FileFilters.QfsFileFilter);
        if (!fin.Success) return;
        var fout = await dialogService.GetFileSavePath(St.Save, St2.SveMessage, FileFilters.FshFileFilter);
